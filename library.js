...
var scaling= function(chartdata,xaxisticks,yaxisticks,max,min,index){
        ...
        for(var k =0;k<chartdata.data.length;k++){
                ...
                ycord = (chartHeight - ((chartdata.data[k].value[index]-min)*ratio))+marginxy;
       ...
       for(var i=0;i<yaxisticks;i++){
                    tickY =chartHeight-((parseInt(divisiony)*(i+1)))+marginxy;
                    this.createLines(url,svg,marginxy,tickY,marginxy-15,tickY);
                    var text=this.chartdata.data[i].value[index];
                    this.createText(url,svg,marginxy-30,tickY,text);                    
         } 
